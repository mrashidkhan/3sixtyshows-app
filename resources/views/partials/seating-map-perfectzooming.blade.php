...
.sm-zoom-btns{position:absolute;bottom:16px;right:16px;z-index:25;display:none;flex-direction:column;border-radius:8px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.3)}
        ...
        loadSDK(function () {
            new seatsio.SeatingChart({
                ...
                onChartRendered: function (chart) {
                    ...
                    /* Wire Reset View button to chart.resetView() */
                    ...
                    /* Show +/- zoom buttons with proper zoom direction */
                    document.getElementById('smZoomBtns').style.display = 'flex';

                    /* ── Zoom: scale #sm-full from top-left, parent clips/scrolls ──
                       zoomLevel 1 = 100%, 1.5 = 150% etc.
                       We expand the logical size so the scrollable
                       area matches the scaled content exactly.         */
                    var zoomLevel = 1;
                    var minZoom   = 0.5;
                    var maxZoom   = 3.0;
                    var zoomStep  = 0.2;
                    var smFull    = document.getElementById('sm-full');
                    var smChart   = smFull ? smFull.parentElement : null;

                    function applyZoom() {
                        if (!smFull || !smChart) return;
                        /* Scale the map from top-left corner */
                        smFull.style.transform       = 'scale(' + zoomLevel + ')';
                        smFull.style.transformOrigin = 'top left';
                        smFull.style.transition      = 'transform 0.18s ease';
                        /* Expand the scroll container so all scaled content
