...
.sm-body{flex:1;min-height:0;display:flex;overflow:hidden}
.sm-chart{flex:1;min-width:0;position:relative;background:#f5f5f5;overflow:auto;min-height:550px;max-height:70vh}

/* seats.io renders here — with scrolling support */
#sm-full-wrap{display:inline-block;min-width:100%}
...
/* Red scrollbar — immediately on map's right edge */
.sm-chart::-webkit-scrollbar{width:8px;height:8px}
.sm-chart::-webkit-scrollbar-track{background:#f0f0f0}
...
.sm-chart{scrollbar-width:thin;scrollbar-color:#C8102E #f0f0f0}
...
(function () {
    ...
    function applyZoom() {
        ...
        var baseW = smChart.clientWidth;
        var baseH = 600;
        /* Scale sm-full visually */
        smFull.style.transform       = 'scale(' + zoomLevel + ')';
        smFull.style.transformOrigin = 'top left';
        smFull.style.transition      = 'transform 0.18s ease';
        /* Set wrapper to actual scaled pixel size so scrollbar tracks correctly */
        var wrap = document.getElementById('sm-full-wrap');
        if (wrap) {
