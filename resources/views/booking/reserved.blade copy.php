...
{{-- Public reserved seating booking page for seats.io shows --}}

@extends('layouts.master') {{-- adjust to your public layout --}}
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-4" id="selection-summary" style="display: none;">
                ...
                <div class="card-body p-0">
                    <table class="table table-sm mb-0" id="selected-seats-table">
                        <thead class="thead-light">
                            <tr>
                                <th>Seat</th>
                                <th>Category</th>
            ...
            <div class="card" id="checkout-form-card" style="display: none;">
                ...
                <div class="card-body">
                    <form id="booking-form"
                        ...
                        {{-- Hidden: hold token (server-issued, matches session) --}}
                        <input type="hidden" name="hold_token" id="form-hold-token"
                               value="{{ $holdToken }}">

                        {{-- Hidden: selected seats JSON (populated by JS) --}}
...
(function () {
    // -----------------------------------------------------------------------
    // Configuration passed from PHP (no secret key here — public key only)
    // -----------------------------------------------------------------------
    ...
    // Price map from TicketType → seatsio_category_key
    const PRICE_MAP = {
        @foreach($ticketTypes as $tt)
    ...
    // -----------------------------------------------------------------------
    // Initialise seats.io seating chart
    // -----------------------------------------------------------------------
    const chart = new window.SeatsioSeatingChart({
        ...
        // Show prices from our PRICE_MAP on the widget tooltips
        ...
        // Called when a seat is selected by the user
        ...
        onObjectDeselected: function (deselectedObject) {
            selectedObjects = selectedObjects.filter(
                o => o.id !== deselectedObject.id
            );
            updateSummary();
        },

        // Called when hold token is about to expire — we refresh server-side
        onHoldTokenExpired: function () {
            alert('Your seat hold has expired. The page will reload so you can reselect.');
    ...
    // -----------------------------------------------------------------------
    // Update the right-panel summary when selection changes
    // -----------------------------------------------------------------------
    function updateSummary() {
        ...
        selectedObjects.forEach(function (obj) {
            ...
            row.innerHTML = '<td>' + (obj.label || obj.id) + '</td>'
                          + '<td>' + catLabel + '</td>'
        ...
        // Show/hide panels based on whether anything is selected
        ...
        // Store serialised seat data for the form
        hiddenInput.value = JSON.stringify(selectedObjects.map(obj => ({
            id:            obj.id,
            label:         obj.label,
            categoryLabel: obj.categoryLabel || '',
            category: {
                key:   obj.category ? obj.category.key   : '',
                label: obj.category ? obj.category.label : '',
            },
            labels: {
                section: obj.labels ? obj.labels.section : null,
                parent:  obj.labels ? obj.labels.parent  : null,
                own:     obj.labels ? obj.labels.own     : null,
            },
        })));
    }

    // -----------------------------------------------------------------------
    // Hold token countdown timer (15 minutes)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Auto-refresh hold token every 10 minutes via AJAX
    // -----------------------------------------------------------------------
    setInterval(function () {
        ...
        .then(data => {
            if (data.success) {
                secondsLeft = data.expires_in_minutes * 60;
            }
        })
